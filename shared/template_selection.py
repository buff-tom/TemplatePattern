...
def select_body_reference_template_with_fallback(body, candidates, weights=None):
    ...
    spans = {k: max(max(r[k] for r in candidates.values())-min(r[k] for r in candidates.values()), 1.) for k in weights}
    ...
    outside = []
    for k in weights:
        lo, hi = min(r[k] for r in candidates.values()), max(r[k] for r in candidates.values())
        if not lo <= body[k] <= hi:
